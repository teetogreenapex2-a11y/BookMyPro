...
export async function GET(req: Request, { params }: { params: { slug: string } }) {
  ...
  const playerMemberships = await prisma.membership.findMany({
    // Same exclusion as the Customers page (app/[slug]/customers/page.tsx)
    // - a sandbox prospect must never show up in the real player search
    // used to attach a booking to an existing customer. This was missing
    // here even though it's set correctly on Customers itself.
