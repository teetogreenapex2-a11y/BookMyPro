...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      // Genuinely helpful for tracking down exactly which client the
      // token's real audience was, temporarily surfaced to the client
      // response itself since there's no way to see server logs from a
      // phone. Safe to drop once this is sorted out.
      return NextResponse.json({ error: `Invalid Google token: ${err instanceof Error ? err.message : String(err)}` }, { status: 401 });
    }
    if (!payload?.email) {
    ...
    console.error("Native Google sign-in - unexpected failure:", err);
    // Same reasoning as the inner catch above - temporarily surfacing
    // real detail since there's no other way to see this from a phone.
    return NextResponse.json({ error: `Sign-in failed: ${err instanceof Error ? err.message : String(err)}` }, { status: 500 });
  }
}
